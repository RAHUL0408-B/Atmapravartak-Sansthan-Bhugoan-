...
import { saveCustomLocation, getCustomLocations } from '../services/locationService';
...
const MemberForm = () => {
    ...
    useEffect(() => {
        if (formData.district && formData.taluka) {
            ...
            const staticCities = selectedTaluka ? selectedTaluka.cities : [];
            
            // Fetch and merge custom cities "autosaved" by other users
            getCustomLocations(formData.district, formData.taluka).then(customCities => {
                const merged = [...new Set([...staticCities, ...customCities])];
                setCities(merged);
            });
        } else {
            setCities([]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Autosave manual city/village for future users
            if (manualEntry.city && formData.city) {
                await saveCustomLocation(formData.district, formData.taluka, formData.city);
            }
        ...
        <div className="card" style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            <form onSubmit={handleSubmit}>
                <div className="form-grid">
                    ...
                    <div className="form-group-row full-width">
                        <div>
                            <label style={labelStyle}>गाव / शहर (Village/City) *</label>
